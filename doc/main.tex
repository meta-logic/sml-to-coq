...
\end{coq}
Therefore, nested functor applications are unfolded into multiple declarations:
\begin{coq}
...
\arraycolsep=10pt\def\arraystretch{3}
\begin{array}{cc}
\infer[\mathsf{StrBind}_{\smles{struct}}]{\smle{S = struct sd end} \stogstrdec \coqe{Module S := sd\\g}}{
  \smle{sd} \stogstrdec \coqe{sd\\g}
}\\
\infer[\mathsf{StrExp}_{\smles{id}}]{\smle{S = SId} \stogstrbind \coqe{Module S := SId\\g}}{
  \smle{SId} \stogstrid \coqe{SId\\g}
}\\
\infer[\mathsf{StrExp}_{\smles{colon}}]{\smle{S : SIG = SId} \stogstrexp \coqe{Module S : SIG\\g := SId\\g}}{
  \smle{SId} \stogstrid \coqe{SId\\g}&
