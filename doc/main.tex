...
\arraycolsep=10pt\def\arraystretch{3}
\begin{array}{c}
\infer[\mathsf{TyExp}_{\smles{TyVar}}]{\smle{'t} \stogty \coqe{_t}}{
...
\arraycolsep=10pt\def\arraystretch{3}
\begin{array}{cc}
\infer[\mathsf{TyExp}_{()}]{\smle{(t)} \stogty \coqe{(t\\g)}}{
  \smle{t} \stogty \coqe{t\\g}
}
&
\infer[\mathsf{TyExp}_{\smles{->}}]{\smle{t_1 -> t_2} \stogty \coqe{t\\g_1 -> t\\g_2}}{
...
\arraycolsep=10pt\def\arraystretch{3}
\begin{array}{c}
\infer[\mathsf{TyExp}_{\smles{TyCon}}]{\smle{t_1, \\dots ,t_n con} \stogty \coqe{con\\g t\\g_1 ... t\\g_n}}{
...
\\
\infer[\mathsf{TyExp}_{\smles{*}}]{\smle{(t_1 * t_2 * \\dots * t_n)} \stogty 
...
\\
\infer[\mathsf{TyExp}_{rec}]{\smle{\{ l_1: t_1, \\dots, l_n: t_n \}} \stogty \coqe!\{ l\\g_1: t\\g_1; ...; l\\g_n: t\\g_n \}!}{
...
\end{array}
\]
\giselle{Why is the \%\coqes{type} needed in the translation of tuple types?}

Notice that type constructors may take 0 arguments 
(rule $\mathsf{TyExp}_{\smles{TyCon}}$), but tuple and record types 
must consist of at least one type 
(rules $\mathsf{TyExp}_{\smles{*}}$ and $\mathsf{TyExp}_{rec}$).
...
In Coq, patterns cannot be typed (error ``Casts are not supported in
this pattern''). Therefore, we ignore types in patterns coming from
SML (rules $\mathsf{Pat}_{\smles{Ty}}$, $\mathsf{Pat}_{\smles{as/ty}}$, 
and $\mathsf{Pat}_{\smles{op/as/ty}}$).
\giselle{Can this be problematic in any way?}
...
&
\infer[\mathsf{Pat}_{\smles{Ty}}]{\smle{p : t} \stogpat \coqe{p\\g}}{
  \smle{p} \stogpat \coqe{p\\g}
}
\\
...
\\
\infer[\mathsf{Pat}_{\smles{as/ty}}]{\smle{id : t as p} \stogpat \coqe{p\\g as id\\g}}{
  \smle{id} \stogvid \coqe{id\\g}
  &
  \smle{p} \stogatpat \coqe{p\\g}
}
