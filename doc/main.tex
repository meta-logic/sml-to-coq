...
\newcommand\stogtypbind{\;\stog_\mathtt{TypBind}\;}
...
TODO: This is the place to document how we translate constants. Things we need
to mention here:

\begin{itemize}
  \item We create the notation \# for characters in Coq to look like SML
  characters\giselle{I think \# stands for something in Coq already, double
  check.}

  \item We use \coqe{\%float} for translating real numbers because numbers line
  6 can be considered Z or floats, depending on which scope was imported last.
  (In SML, 6 is always \smle{int}, and 6.0 is \smle{real}).
...
&
\infer[\mathsf{TyCon}_{\smles{bool}}]{\smle{bool} \stogtycon \coqe{bool}}{}
\\
\infer[\mathsf{TyCon}_{\smles{int}}]{\smle{int} \stogtycon \coqe{int}}{}
&
\infer[\mathsf{TyCon}_{\smles{word}}]{\smle{word} \stogtycon \coqe{word}}{}
\\
\infer[\mathsf{TyCon}_{\smles{real}}]{\smle{real} \stogtycon \coqe{real}}{}
&
\infer[\mathsf{TyCon}_{\smles{string}}]{\smle{string} \stogtycon \coqe{string}}{}
\\
\infer[\mathsf{TyCon}_{\smles{char}}]{\smle{char} \stogtycon \coqe{char}}{}
&
\infer[\mathsf{TyCon}_{\smles{list}}]{\smle{list} \stogtycon \coqe{list}}{}
\\
\infer[\mathsf{TyCon}_{\smles{ref}}]{\smle{ref} \stogtycon \coqe{??}}{}
&
\infer[\mathsf{TyCon}_{\smles{exn}}]{\smle{exn} \stogtycon \coqe{??}}{}
\\
\end{array}
\]

TODO: missing valid names for type constructors (e.g. tree, stream, etc).

\subsection{Names and identifiers}

TODO: In this section we mention which names need modification. From what I
remember, these are:

\begin{itemize}
  \item Type variables (which start with \_ in Coq and ' in SML)
  
  \item Identifiers that clash with Coq keywords\giselle{Still needs to be
  implemented}
...
\arraycolsep=15pt\def\arraystretch{3}
\begin{array}{cc}
\infern{\texttt{e is SCon}}{\smle{e} \stogatexp \coqe{e}}{AtExp}
\qquad
\infern{\texttt{e is VId}}{\smle{e} \stogatexp \coqe{e}}{AtExp}
\\
\infern{\smle{e} \stogexp coqe{t}\quad \smle{d}\stogdec \coqe{Definition d'}}{
  \smle{let d in e end} \stogatexp \coqe{let d' in t}
}{AtExp}
\\
\infern{
  \begin{gathered}
  ...
  \smle{let d in e end} \stogatexp \coqe!let d$_1$ in ... in let d$_n$ in t!
}{\mathsf{AtExp}}
\\
\infern{\smle{e} \stogexp \coqe{t}}{\smle{(e)} \stogatexp \coqe{(t)}}{AtExp}
\\
\infern{\smle{e}_1 \stogexp \coqe{t}_1 & \dots & \smle{e}_n \stogexp \coqe{t}_n}{
  \smle!(e$_1$, ... , e$_n$)! \stogatexp \coqe!(t$_1$, ... ,t$_n$)!
}{AtExp}
\\
\infern{\smle{e}_1 \stogexp \coqe{t}_1 & \dots & \smle{e}_n \stogexp \coqe{t}_n}{
  \smle![e$_1$, ... , e$_n$]! \stogatexp \coqe![t$_1$; ... ;t$_n$]!
...
\arraycolsep=15pt\def\arraystretch{3}
\begin{array}{cc}
\infer[\mathsf{AtExp}_1]{\smle{e} \stogexp \coqe{e'}}{
...
\giselle{The resulting translations of \smle{andalso} and \smle{orelse} are not
in the Gallina AST. Why?}
...
\begin{array}{cc}
\infer[\mathsf{TyVar}]{\smle{'X} \stogty \coqe{\_X}}{X \text{ is alphanumeric}}
&
\infer[\mathsf{TyRow}]{\smle{\{ s \}} \stogty \coqe{\{ g \}}}{s \stog g}
\\
\infer[\mathsf{TyCon}_p]{\smle{s t} \stogty \coqe{h g}}{
  s \stogty g
  &
  t \stogtycon h
}
&
\infer[\mathsf{Ty}_{\smles{->}}]{t_1 \smle{->} t_2 \stogty g_1 \coqe{->} g_2}{
  t_1 \stogty g_1
  &
  t_2 \stogty g_2}
\\
\infer[\mathsf{Ty}_{()}]{\smle!( $t$ )! \stogty \coqe!( $g$ )!}{
  t \stogty g
}
&
\infer[\mathsf{Ty}_*]{\smle!($t_1$ * $t_2$ * ... * $t_n$)! \stogty 
                      \coqe!($g_1$ * $g_2$ * ... * $g_n$)\%type!}
{
  t_i \stogty g_i & \forall i. 1 \leq i \leq n
}
\\
\end{array}
\]
...
\begin{itemize}
  ...
  am not sure if the type parameters are translated into Coq... \laila{type
  parameters are indeed translated, but the type constructor comes in the
  beginning so 'a list become list \_a} 
  
  \item Maybe move the translation of type variables to the identifiers part?
